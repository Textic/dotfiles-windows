...
function Get-CurrentStatus {
    try {
        ...
        if ($AutoEnd -eq "1" -and $HungTimeout -eq "1000" -and $WaitTimeout -eq "2000" -and $HooksTimeout -eq "1000" -and $MenuDelay -eq "10") {
            return $true
        }
        return $false
    } catch {
...
if ($Enable) {
    ...
    try {
        $Path = "HKCU:\Control Panel\Desktop"
        Set-ItemProperty -Path $Path -Name "AutoEndTasks" -Value "1" -PropertyType String -Force
        Set-ItemProperty -Path $Path -Name "HungAppTimeout" -Value "1000" -PropertyType String -Force
        Set-ItemProperty -Path $Path -Name "WaitToKillAppTimeout" -Value "2000" -PropertyType String -Force
        Set-ItemProperty -Path $Path -Name "LowLevelHooksTimeout" -Value "1000" -PropertyType String -Force
        Set-ItemProperty -Path $Path -Name "MenuShowDelay" -Value "10" -PropertyType String -Force
...
if ($Disable) {
    ...
    try {
        $Path = "HKCU:\Control Panel\Desktop"
        Set-ItemProperty -Path $Path -Name "MenuShowDelay" -Value "400" -PropertyType String -Force
